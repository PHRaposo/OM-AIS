...
(defun interval-inversions (intervals)
 (om::om* 100 (mapcar #'(lambda (input)  
  (if (> input 0)
      (* (- 12 input) -1)
      (+ input 12))) (om::om/ intervals 100))))

(defun remove-interval-inversions (intervals-lists counter init-length)
 (let* ((results 
           (om::x-append 
            (remove-if #'(lambda (input) 
                                (equal (interval-inversions (first intervals-lists)) input)) (cdr intervals-lists))
           (list (first intervals-lists)))))
(if (equal init-length counter)
    (write results)
    (remove-interval-inversions results (1+ counter) init-length))))

(defun get-range (intervals-list)
 (let ((get-midics (om::dx->x 4800 intervals-list)))
...
(let* ((space (om::om* 100
         (om::mat-trans (list
                                   ...
                                   (mapcar #'(lambda (input)
                                    (if (> input 0)
                                        (* (- 12 input) -1)
                                        (+ input 12))) (om::x->dx ais))))))

      (rule ;no duplicate abs-intervals
        (omcs::wildcard-rule
         (lambda (input1)
          (not (member (abs input1) (cdr (om::om-abs (omcs::rev-partial-solution)))))))))

(omcs::pmc-engine space (list rule) 
:heuristic-rules nil :fwc-rules nil :sols-mode :all :rnd? nil :print-fl nil)))
...
(om::defmethod! AIS->MELODIC-CONTOURS ((AIS list) &optional (max-range 6600))
        :initvals '( '(0 1 3 2 7 10 8 4 11 5 9 6) 6600)
  	:indoc '("AIS") 
  	:icon 06
  	:doc "Returns a list of lists of eleven intervals, in midicents, containing all 64 possible melodic contours of the given all intervals series, with intervals no higher than a octave. The optional argument is for selecting only the melodic contours with the given maximun range between the lowest and highest note."
   (sort-by-range (get-ais-intervals AIS) max-range))
